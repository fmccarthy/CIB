...
# this is originally Moritz Munchmeyer's code that I have been editing and using to implement CIB calculations.
# most of it i do not use at all.
# this is a very old version of the code.

#all i use this for is halo bias and nfn, really. I could probably just delete the rest of the commands.
#in particular most things won't even work anyway.
...
class HaloModel(object):
    ...
    def __init__(self):
        ...
        self.npts=1000#1000#1500#2000 #400       #1000     # Number of points equally spaced in log_10 m to sample. Default = 400
        self.logmmin=9.95#8#9.95  #4          # log_10 m/M_\odot of the minimum mass halo to sample. for kmax=1000, should be > 4. Default = 4.
        ...
        kmax = 100.
        kmin = 1e-5
        self.k = np.logspace(np.log10(kmin),np.log10(kmax),num=2000)
        self.z = [] #filled later
    ...
    def _setup_k_z_m_grid_functions(self,z,mthreshHOD=mthreshHODdefault,need_ukm=True):
        ...
        for j in range(0,self.z.shape[0]):
            ...
            self.conc[:,j] = 7.85*(( self.h*0.5*10**(self.logm-12) )**(-0.081))*(1+self.z[j])**(-0.71) # Duffy08 all model from 1005.0411
            # Cube of the virial radius. Note, this is a physical distance, not a comoving distance.
            self.rvir3[:,j] = 3.*(10**self.logm)/(4.*np.pi*self.deltav*self.rhocrit*(1+self.z[j])**3.)
            
         
        mhalos=np.exp(self.lnms)
        zslessthan25=self.z.copy()
        zslessthan25[zslessthan25>2.5]=2.5
        A=0.186*(1+zslessthan25)**-0.14
        a=1.47*(1+zslessthan25)**-0.06
        logpower=-(0.75/np.log10(200/75))**1.2
        bpower=10**logpower
        b=2.57*(1+zslessthan25)**bpower
        c=1.19
        self.fsigma=A*((np.sqrt(self.sigmam2)/b)**-a+1)*np.exp(-c/self.sigmam2)# self.c1*np.sqrt(2.*self.c2/np.pi)*(1.+(self.sigmam2[i,j]/(self.c2*self.deltac**2))**self.p)*(self.deltac/np.sqrt(self.sigmam2[i,j]))*np.exp(-0.5*self.c2*self.deltac**2/self.sigmam2[i,j])
    ...
    def mrvir_to_mrdelta(self,delta,zindex,c,rv,mv):
        ...
        fn = np.abs(rdeltavals**3 - 3.*mv*mcr/(4.*np.pi*delta*rhocrit*mc*(1+z[zindex])**3))
    ...
    def gasprofile(self,c,rv,mv,rvals,rstar):
        ...
        gasrescale=(nfw[indexxstar]/gas[indexxstar])*rhos*(ombh2/omch2)
        return np.piecewise(xvals,[xvals<=xstar,xvals>xstar],[lambda xvals: gasrescale*((1. - (3./eta0)*((gamma-1.)/gamma)*(c/mc)*(1.-np.log(xvals+1.)/(xvals)))**(1./(gamma-1.))), lambda xvals: rhos*(ombh2/omch2)/(xvals*(1+xvals)**2)])
    ...
    # Note: All distances are physical distances, not comoving distances.
    def gasprofile_battaglia_AGN(self,c,rv,mv,rvals,zindex):    
        m200, r200 = mrvir_to_mrdelta(200,zindex,c,rv,mv)
        rho0 = 4000.*((m200/(10**(14)))**(0.29))*(1+z[zindex])**(-0.66)
        alpha = 0.88*((m200/(10**(14)))**(-0.03))*(1+z[zindex])**(0.19)
        beta = 3.83*((m200/(10**(14)))**(0.04))*(1+z[zindex])**(-0.025)
        xc = 0.5
        gamma = -0.2
        rhocritz = rhocrit*(1+z[zindex])**3
        xxs = rvals/r200 
        ans = rhocritz*rho0*((xxs/xc)**gamma)*( 1. +(xxs/xc)**alpha )**(-(beta+gamma)/alpha)
        return ans
    ...
    def gasprofile_battaglia_SH(self,c,rv,mv,rvals,zindex):    
        m200, r200 = mrvir_to_mrdelta(200,zindex,c,rv,mv)
        rho0 = 19000.*((m200/(10**(14)))**(0.09))*(1+z[zindex])**(-0.95)
        alpha = 0.70*((m200/(10**(14)))**(-0.017))*(1+z[zindex])**(0.27)
        beta = 4.43*((m200/(10**(14)))**(0.005))*(1+z[zindex])**(0.037)
        xc = 0.5
        gamma = -0.2
        rhocritz = rhocrit*(1+z[zindex])**3
        xxs = rvals/r200
    ...
    # def calc_gasprofiles(): #TODO (MM)
        ...
        #ukgas = np.zeros([npts,nzeds,nks])
        #ukgas_AGN = np.zeros([npts,nzeds,nks])
        #ukgas_SH = np.zeros([npts,nzeds,nks])
        #
        #for j in range(0,z.shape[0]):
        #    for i in range(0,logm.shape[0]):
        #        c=conc[i,j]
        #        rv=(rvir3[i,j])**(0.3333333)
        #        rvals=np.linspace(0.0001,rv,10000)
        #        rstar=rv
        #        mv=10**logm[i]
        #        #mgas=4.*np.pi*np.trapz((rvals**2)*gasprofile(c,rv,mv,rvals,rstar),rvals)
        #        mgas_AGN = 4.*np.pi*np.trapz((rvals**2)*gasprofile_battaglia_AGN(c,rv,mv,rvals,j),rvals)
        #        mgas_SH = 4.*np.pi*np.trapz((rvals**2)*gasprofile_battaglia_SH(c,rv,mv,rvals,j),rvals)
        #        for q in range(0,k.shape[0]):
        #            kphys=k[q]*(1+z[j])
        #            ukgas[i,j,q] = np.trapz(4.*np.pi*(rvals**2)*(np.sin(kphys*rvals)/(kphys*rvals))*gasprofile(c,rv,mv,rvals,rstar)/mgas,rvals)
        #            ukgas_AGN[i,j,q] = np.trapz(4.*np.pi*(rvals**2)*(np.sin(kphys*rvals)/(kphys*rvals))*gasprofile_battaglia_AGN(c,rv,mv,rvals,j)/mgas_AGN,rvals)
        #            ukgas_SH[i,j,q] = np.trapz(4.*np.pi*(rvals**2)*(np.sin(kphys*rvals)/(kphys*rvals))*gasprofile_battaglia_SH(c,rv,mv,rvals,j)/mgas_SH,rvals)
        #
        #np.save('gasprofile_universal', ukgas)
        #np.save('gasprofile_AGN', ukgas_AGN)
        #np.save('gasprofile_SH', ukgas_SH)
        #np.savez('gasprofile_sampling.npz',logm_precomp=self.logm,z_precomp=self.z,k_precomp=self.k) #TODO
    ...
    def _Ponehalo(self,z,lnms,k,uk1,uk2,nfn,rhomatter,spec,frequency=None):#Fiona: added optional frequency argument for CIB
        ...
        #need uk1 to correspond to ukm; do not need uk2.
            ...
            if kszpsz_config.CIB_model=="minimally_empiricial":
                ...
                central_flux=np.transpose(CIB_Planck.Scentral(frequency,z[:,np.newaxis],np.exp(lnms)))
        ...
        if spec in ['CIBgas']: #Fiona 
            ...
            onehalointegrand =remote_spectra.chifromz(z[np.newaxis,:,np.newaxis])**2*np.exp(lnms[:,np.newaxis,np.newaxis])*(nfn[:,:,np.newaxis]*np.exp(lnms[:,np.newaxis,np.newaxis])/(rhomatter))*(nfn[:,:,np.newaxis]*central_flux[:,:,np.newaxis] + satflux[:,:,np.newaxis]*uk1[:,:,:])*uk2[:,:,:]
    ...
    def _Ptwohalo(self,z,lnms,k,pk,uk1,uk2,nfn,rhomatter,halobias,spec,frequency=None):#Fiona: added optional frequency argument
        ...
        print("integrated in",time.time()-t1,"seconds")
