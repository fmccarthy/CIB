...
#this is Juan Cayuso's code. I use very little of it.
...
ag = np.logspace(np.log10(conf.adec), 0, 100)
...
def aeq(Omega_b, Omega_c, h, Omega_r=conf.Omega_r):
    """Scale factor at matter radiation equality"""
    return Omega_r / (Omega_b + Omega_c)
...
def Omega_L(Omega_b, Omega_c, Omega_K, h, Omega_r=conf.Omega_r):
    """Omega_L in terms of Omega_b, Omega_c, and K imposed by consistency"""
    return 1 - Omega_K - (Omega_b + Omega_c) - Omega_r


def E(a, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=conf.Omega_r):
...
def H_config(a, conf):
    ...
    return H(a, conf.Omega_b, conf.Omega_c, conf.w, conf.wa, conf.Omega_K, conf.h)


def dEda(a, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=conf.Omega_r) :
    """
    ...
    d = -3.0*(Omega_b + Omega_c) / a**4 - 2.0*Omega_K / a**3 \
        ...
        - 4.0*Omega_r / a**5
    derv_E = d / (2.0 * E(a, Omega_b, Omega_c, w, wa, Omega_K, h))
    return derv_E
...
def dHdt_config(a, conf) :
    """(non-conformal) time-derivative of hubble parameter"""
    return dHdt(a, conf.Omega_b, conf.Omega_c, conf.w, conf.wa, conf.Omega_K, conf.h)
...
def tau_z(z, Omega_b, h):
    ...
    tau = integrate.cumtrapz(integrand, chi_grid, initial=0)
...
def Integrand_GF(s, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    """
    Integrand = 1 / (s * E(s, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=0.0))**3
    return Integrand
...
def Integrand_GF_ODE(y, a, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    """
    f = 1.0e13 * (a * E(a, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=0.0))**-3
    return f
...
def Integral_GF_ODE(Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    Integral = integrate.odeint(Integrand_GF_ODE,
                                Integral_GF(ag[0], Omega_b, Omega_c,
                                            w, wa, Omega_K, h) * (10**13),
                                ag, args=(Omega_b, Omega_c, w, wa, Omega_K, h,))

    return Integral * 10**-13
...
def GF(Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    l = 5.0/2.0 * (Omega_b + Omega_c) * \
        E(ag, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=0.0) / ag
...
def derv_Dpsi(Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    """
    y = ag / aeq(Omega_b, Omega_c, h)
    Eag = E(ag, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=0.0)
    dEdag = dEda(ag, Omega_b, Omega_c, w, wa, Omega_K, h, Omega_r=0.0)
    fancy_9_10 = (16.0*np.sqrt(1.0 + y) + 9.0*y**3 + 2.0*y**2 - 8.0*y - 16.0) / (10.0*y**3)
...
def G_Dopp_ksz(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    return G_Dopp_ksz
def G_Dopp_ksz_nolocaldoppler(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    """kSZ Integral kernel for the Doppler effect"""
    chidec = Chia(Omega_b, Omega_c, w, wa, Omega_K, h)[0]
    chie = Chia_inter(Omega_b, Omega_c, w, wa, Omega_K, h)(az(ze))
    G_Dopp_ksz = k * Dv(Omega_b, Omega_c, w, wa, Omega_K, h)[0] * (special.spherical_jn(0, k * (
...
def G_ISW_ksz(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    integrand = special.spherical_jn(
        1, s2) * derv_Dpsi_inter(Omega_b, Omega_c, w, wa, Omega_K, h)(a)
    g_isw_ksz = 6 * integrate.cumtrapz(integrand, a, initial=0)
...
def G_ISW_psz(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    g_isw_psz = -8 * np.pi * integrate.cumtrapz(integrand, a, initial=0)
...
def G_ISW_CMB(k, l, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    g_isw_CMB = 8 * np.pi * \
        (1j**l) * integrate.cumtrapz(integrand, a, initial=0)
...
def G_ksz(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    k_s = np.logspace(-5, 0, 500) # Todo: Why these k's?
  
    G_s = G_SW_ksz(k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h ) \
        + G_Dopp_ksz( k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h ) \
        + G_ISW_ksz(k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h)
    G_int = Dv_inter = interp1d(
        k_s, G_s, kind="cubic", bounds_error=False, fill_value='extrapolate')

    return G_int(k)

def G_ksz_nolocaldoppler(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    """Sum of kSZ integral kernels"""
    k_s = np.logspace(-5, 0, 500) # Todo: Why these k's?
  
    G_s = G_SW_ksz(k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h ) \
        + G_Dopp_ksz_nolocaldoppler( k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h ) \
        + G_ISW_ksz(k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h)
    G_int = Dv_inter = interp1d(
        k_s, G_s, kind="cubic", bounds_error=False, fill_value='extrapolate')
...
def G_ksz_localDopp(k, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    k_s = np.logspace(-5, 0, 500)
    G_s = G_localDopp_ksz(k_s, ze, Omega_b, Omega_c, w, wa, Omega_K, h)
    G_int = Dv_inter = interp1d(
        k_s, G_s, kind="cubic", bounds_error=False, fill_value='extrapolate')
...
def Get_Windowed_Transfer(Transfer, redshifts, redshift_weights, k, L, *args, **kwargs) :
    """
    Generic function to compute windowed transfer functions; computes:

      Transfer(k, L, z, Omega_b, Omega_c, w, wa, Omega_K, h, kwargs)    
    ...
    *args and **kwargs are passed through to transfer.
    ...
    if N_samples == 1:
        ...
        Window_norm = np.trapz(redshift_weights, redshifts)
        ...
        Transfer_windowed = np.trapz(T_samples, redshifts, axis=0) / Window_norm
...
def Transfer_ksz_redshift(k, L, ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    for l in L:
        if l == 0:
            transfer_ksz[:, 0] = 0.0 + 1j * 0.0
        else:
            c = (4 * np.pi * (1j)**l) / (2 * l + 1)
            transfer_ksz[:, l] = c * (l * special.spherical_jn(l - 1, k * Chie) - (
        ...
        for l in L:
            if l == 0:
             #   T_samples[:, 0, j] = 0.0 + 1j * 0.0
              c = (4 * np.pi * (1j)**l) / (2 * l + 1)
              T_samples[:, l, j] = c * ( - (
                    l + 1) * special.spherical_jn(l + 1, k * Chie)) * Tk * Ker #fiona
            else:
                c = (4 * np.pi * (1j)**l) / (2 * l + 1)
...
def Transfer_ksz_bin_localdoppler(N, n, B, k, L, Omega_b, Omega_c, w, wa, Omega_K, h):
    """
    KSZ transfer function for a given binning scheme
    """
    T_samples = np.zeros((len(k), len(L),) + (n,), dtype=np.complex64)
    ...
    for j in np.arange(n):
        t1=time.time()
      #  print(j)
        Ker = G_ksz_localDopp(k, z_samples[j], Omega_b, Omega_c, w, wa, Omega_K, h)
        Chie = Chia_inter(Omega_b, Omega_c, w, wa,
                          Omega_K, h)(az(z_samples[j]))
        #print(Chie)
        #print(Ker.shape)
    ...
    return transfer_ksz
def Transfer_ksz_bin_nolocaldoppler(N, n, B, k, L, Omega_b, Omega_c, w, wa, Omega_K, h):
    """
    KSZ transfer function for a given binning scheme
    """
    T_samples = np.zeros((len(k), len(L),) + (n,), dtype=np.complex64)
    ...
    for j in np.arange(n):
      #  print(j)
        t1=time.time()
      #  print(j)
        Ker = G_ksz_nolocaldoppler(k, z_samples[j], Omega_b, Omega_c, w, wa, Omega_K, h)
        Chie = Chia_inter(Omega_b, Omega_c, w, wa,
                          Omega_K, h)(az(z_samples[j]))
        #print(Chie)
        #print(Ker.shape)
    ...
    return transfer_ksz
def Transfer_ksz_bin_arrays(N, n, B, k, L, Omega_b, Omega_c, w, wa, Omega_K, h):
    """
    KSZ transfer function for a given binning scheme
    """
    T_samples = np.zeros((len(k), len(L),) + (n,), dtype=np.complex64)
    ...
    for j in np.arange(n):
       ...
       # for l in L:
        ...
        T_samples[:, :, j] = c * (L * special.spherical_jn(L - 1, k[:,np.newaxis]  * Chie) - (
                    L + 1) * special.spherical_jn(L + 1, k[:,np.newaxis] * Chie)) * Tk[:,np.newaxis] * Ker[:,np.newaxis]
        print(time.time()-t1,"seconds")
...
def Transfer_ksz_bin_arrays2(N, n, B, k, L, Omega_b, Omega_c, w, wa, Omega_K, h):
    """
    KSZ transfer function for a given binning scheme
    """
    T_samples = np.zeros((len(k), len(L),) + (n,), dtype=np.complex64)
    ...
    #for j in np.arange(n):
     #   print(j)
    t1=time.time()
    Ker = G_ksz(k[:,np.newaxis], z_samples, Omega_b, Omega_c, w, wa, Omega_K, h)
    Chie = Chia_inter(Omega_b, Omega_c, w, wa,
                          Omega_K, h)(az(z_samples))
        #print(Chie)
    ...
    T_samples[:, :, :] = c[np.newaxis,:,np.newaxis] * (L * special.spherical_jn(L[np.newaxis,:,np.newaxis] - 1, k[:,np.newaxis,np.newaxis]  * Chie) - (
                    L + 1) * special.spherical_jn(L [np.newaxis,:,np.newaxis]+ 1, k[:,np.newaxis,np.newaxis] * Chie)) * Tk[:,np.newaxis,np.newaxis] * Ker[:,np.newaxis,:]
    print(time.time()-t1,"seconds")
...
def Transfer_ksz_windowed(k, L, z_samples, z_sample_weights, config=conf):
    """
    kSZ transfer function averaged in a window
    """
    return Get_Windowed_Transfer(Transfer_ksz_redshift, z_samples, z_sample_weights, k, L,
...
def Transfer_ksz_localDopp_windowed(k, L, z_samples, z_sample_weights, config=conf):
    """
    kSZ transfer function, local doppler only, averaged in a window
    """
    return Get_Windowed_Transfer(Transfer_ksz_localDopp_redshift, z_samples, z_sample_weights, k, L,
        config.Omega_b, config.Omega_c, config.w, config.wa, config.Omega_K, config.h)
    ...
    if ze == 0.0:
        ...
        for l in L:
            if l == 2:
                ...
                transfer_psz[:, l] = c * (1. / 15.) * Tk * Ker
            else:
                transfer_psz[:, l] = 0.0 + 1j * 0.0
...
def Transfer_CMB(k, L, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    for l in L:
        if l < 1:
            transfer_CMB[:, l] = 0.0 + 1j * 0.0
        else:
            Ker = G_CMB(k, l, Omega_b, Omega_c, w, wa, Omega_K, h)
            transfer_CMB[:, l] = Tk * Ker
...
def Transfer_E(k, L, Omega_b, Omega_c, w, wa, Omega_K, h, tau):
    ...
    transfer_E = (integrate.cumtrapz(
        Integrand, Chi_grid, axis=-1, initial=0))[:, :, -1]
...
def Transfer_NGR_redshift(k, L, ze, terms=('all'), config=conf):
    """
    ...
    fixed "f_evol" and "s"
    terms can contain: ('all', 'delta', 'rsd', 'v1v2', 'lens', 'p1', 'p2', 'p3', 'p4', 'isw')
    """
    transfer = np.zeros((len(k),len(L)), dtype = np.complex64)

    if 'all' in terms:
        transfer += Transfer_NGR_all_redshift(k, L, ze, config=config)
    else:
        if 'delta' in terms :
        ...
        if 'lens' in terms :
            transfer += Transfer_NGR_Lensing_redshift(k, L, ze, config=config)
        if 'p1' in terms :
        ...
        if 'p4' in terms :
            transfer += Transfer_NGR_P4_redshift(k, L, ze, config=config)
        if 'isw' in terms :
            transfer += Transfer_NGR_ISW_redshift(k, L, ze, config=config)

    return transfer


def Transfer_NGR_delta_redshift(k, L, ze, config=conf):
    """
    General relativistic number counts ("NGR") transfer function for a given redshift
    Density component
    No window function for now
    fixed "f_evol" and "s"
    ...
    for l in L:
        if l == 0:
            transfer_NGR[:, 0] = 0.0 + 1j * 0.0
        else:
            transfer_NGR[:, l] = b(k, ze) * 4.0*np.pi*(1j)**l * Dpsi * Trans\
              * 2.0*k**2*ae/(3.0*config.Omega_m*H0(config.h)**2) \
...
def Transfer_NGR_RSD_redshift(k, L, ze, config=conf):
    ...
    ae = az(ze)
    Chie = Chia_inter_config(conf)(ae)
    H = H_config(ae, conf)
    Dv = Dv_inter_config(conf)(ae)
    Trans = T_config(k, conf)

    for l in L:
        if l == 0:
            transfer_NGR[:, 0] = 0.0 + 1j * 0.0
        else :
            transfer_NGR[:, l] = 4.0*np.pi*(1j)**l * Trans * (-Dv/(ae*H)) * k**2 \
                * spherical_jn_pp(l, k*Chie)

    return transfer_NGR


def Transfer_NGR_Lensing_redshift(k, L, ze, config=conf):
    """
    General relativistic number counts transfer function for a given redshift
    Lensing component
    """
    ...
    a_integrand = np.linspace(ae, 1.0 - 1.0e-5, config.transfer_integrand_sampling)
    ...
    for l in L:
        if l == 0:
            ...
            integrand = ( Chi_integrand - Chie ) / Chi_integrand / Chie \
                * (2.0 - 5.0*s(k, ze))[:,None] * Dpsi_integrand \
                * special.spherical_jn(l, k[:,None]*Chi_integrand)
            WL = np.trapz(integrand, Chi_integrand)
            transfer_NGR[:, l] = -l*(l+1)*4.0*np.pi*(1j)**l * Trans * WL
...
def Transfer_NGR_V1V2_redshift(k, L, ze, config=conf):
    ...
    Chie = Chia_inter_config(config)(ae)
    Dv = Dv_inter_config(conf)(ae)
    Hubble = H_config(ae, conf)
    Hprime = ae*dHdt_config(ae, conf)
    Trans = T_config(k, config)
    ...
    for l in L:
        if l == 0:
            transfer_NGR[:, 0] = 0.0 + 1j * 0.0
        else: # TODO: transfer function here? Check velocity divergence
            transfer_NGR[:, l] = 4.0*np.pi*(1j)**l * Trans * (-Dv*k**2) * (
...
def Transfer_NGR_P1_redshift(k, L, ze, config=conf):
    ...
    for l in L:
        if l == 0:
            ...
            transfer_NGR[:, l] = 4.0*np.pi*(1j)**l * Trans * special.spherical_jn(l, k*Chie) * (
...
def Transfer_NGR_P4_redshift(k, L, ze, config=conf):
    """
    General relativistic number counts transfer function for a given redshift
    P4 term
    """
    ...
    a_integrand = np.linspace(ae, 1.0 - 1.0e-5, conf.transfer_integrand_sampling)
    Chi_integrand = Chia_inter_config(config)(a_integrand)
    Dpsi_integrand = Dpsi_inter_config(config)(a_integrand)
    ...
    for l in L:
        if l == 0:
            ...
            integrand = 2.0 * (2.0 - 5.0*s(k, ze)[:,None]) / Chie * Dpsi_integrand \
                * special.spherical_jn(l, k[:,None]*Chi_integrand)
            WL = np.trapz(integrand, Chi_integrand)
            transfer_NGR[:, l] = 4.0*np.pi*(1j)**l * Trans * WL

    return transfer_NGR


def Transfer_NGR_ISW_redshift(k, L, ze, config=conf):
    """
    General relativistic number counts transfer function for a given redshift
    ISW term
    """
    ...
    a_integrand = np.linspace(ae, 1.0 - 1.0e-5, conf.transfer_integrand_sampling)
    Hconf_integrand = a_integrand*H_config(a_integrand, config)
    Chi_integrand = Chia_inter_config(config)(a_integrand)
    # d D_psi / da :
    derv_Dpsi_integrand = derv_Dpsi_inter_config(config)(a_integrand)

    for l in L:
        if l == 0:
            transfer_NGR[:, 0] = 0.0 + 1j * 0.0
        else:
            integrand = 2.0*a_integrand*Hconf_integrand*derv_Dpsi_integrand \
                * special.spherical_jn(l, k[:,None]*Chi_integrand)
            WL = np.trapz(integrand, Chi_integrand)
            transfer_NGR[:, l] = 4.0*np.pi*(1j)**l * Trans * prefactor * WL

    return transfer_NGR


def Transfer_NGR_all_redshift(k, L, ze, config=conf):
    """
    General relativistic number counts ("NGR") transfer function for a given redshift
    ...
    Ddelta_m = Dpsi * 2.0*k**2*ae/(3.0*conf.Omega_m*H0(conf.h)**2)

    # Variables to be integrated over
    a_integrand = np.linspace(ae, 1.0 - 1.0e-5, conf.transfer_integrand_sampling)
    ...
    for l in L:
        if l == 0:
            ...
            # TODO: spherical_jn is slow.
            # consider resampling k*Chi to reduce recalculations?
            # ie. k*(2*Chi) = (2*k)*Chi
            integrand = special.spherical_jn(l, k[:,None]*Chi_integrand) * (
                    2.0*Dpsi_integrand * (2.0 - 5.0*s(k,ze))[:,None]/2.0 * (
                        l*(l+1.0)*(Chie-Chi_integrand)/Chie/Chi_integrand + 2.0/Chie )
                    + 2.0*Dpsi_prime_integrand*prefactor[:,None]
                )
            integral_term = np.trapz(integrand, Chi_integrand)
            ...
            # transfer function
            transfer_NGR[:, l] = 4.0*np.pi*(1j)**l * Trans * (
                jl * (
                        b(k,ze)*Ddelta_m + (prefactor+1.0)*Dpsi
...
def Transfer_NGR_windowed(k, L, z_samples, z_sample_weights,
    terms=('all'), config=conf):
    """
    ...
    return Get_Windowed_Transfer(Transfer_NGR_redshift,
        z_samples, z_sample_weights, k, L, terms=terms, config=config)


def get_PkDeltaDelta(ze):
    k = np.logspace(conf.k_min_GR, conf.k_max_GR, conf.k_res_GR)
    a = az(ze)
    ...
    return {
        'k': k,
        'Pk': Dpsi**2 * Trans**2 * (2.0*k**2*a/(3.0*conf.Omega_m*H0(conf.h)**2))**2 \
...
def get_CLNGRNGR_windowed(z_samples, z_sample_weights,
    ...
    k = np.logspace(conf.k_min_GR, conf.k_max_GR, conf.k_res_GR)
    Lv = np.arange(conf.ksz_estim_signal_lmax)
    T_list_1 = [ Transfer_NGR_windowed(k, Lv, z_samples, z_sample_weights, terms=terms_1) ]
    T_list_2 = [ Transfer_NGR_windowed(k, Lv, z_samples, z_sample_weights, terms=terms_2) ]
...
def get_CLNGRNGR(ze, terms_1=('all'), terms_2=('all') ):
    ...
    k = np.logspace(conf.k_min_GR, conf.k_max_GR, conf.k_res_GR)
    Lv = np.arange(conf.ksz_estim_signal_lmax)
    T_list_1 = [ Transfer_NGR_redshift(k, Lv, ze, terms=terms_1) ]
    T_list_2 = [ Transfer_NGR_redshift(k, Lv, ze, terms=terms_2) ]
...
def Transfer_ML_redshift(k, L , ze, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    for l in L:
        if l == 0:
            transfer_ML[:,0] = 0.0 +1j*0.0
        else:
            c = -(4*np.pi*(1j)**l)
            transfer_ML[:,l]= c*special.spherical_jn(l, k*Chie)*Tk*Dv/Chie
...
def Transfer_ML_windowed(k, L, z_samples, z_sample_weights, config=conf):
    """
    Moving lens transfer function averaged using a window
    """
    return Get_Windowed_Transfer(Transfer_ML_redshift, z_samples, z_sample_weights, k, L,
        config.Omega_b, config.Omega_c, config.w, config.wa, config.Omega_K, config.h)


def get_CLvtvt():
    print("Calculating Cl_vtvt...")
    k = np.logspace(conf.k_min, conf.k_max, conf.k_res)
    Lv = np.arange(conf.ksz_estim_signal_lmax)
...
def I_psz(k,ze,l, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    I_psz = 2.*np.pi*np.sqrt(6)*integrate.cumtrapz(integrand, tau, initial=0)
...
def I_Ttens(k, l, Omega_b, Omega_c, w, wa, Omega_K, h):
    ...
    I_Ttens = integrate.cumtrapz(integrand, tau, initial=0)
...
def Transfer_Btens_novis(k, L , Omega_b, Omega_c, w, wa, Omega_K, h, tau):
    ...
    transfer_B = (integrate.cumtrapz(Integrand, Chi_grid, axis=-1, initial=0))[:,:,-1]
...
def Transfer_Etens_novis(k, L , Omega_b, Omega_c, w, wa, Omega_K, h, tau):
    ...
    transfer_E = (integrate.cumtrapz(Integrand, Chi_grid, axis=-1, initial=0))[:,:,-1]
...
def CL_binstens(T_list1, T_list2, k, L, r, As, nt):
    ...
    for i in np.arange(len(T_list1)):
        ...
        for l in L:
            ...
            CL[i,l] = np.real((integrate.cumtrapz(I, k, initial=0))[-1])
    ...
    for i in np.arange(len(T_list)):
        ...
        for j in np.arange(i,len(T_list)):
            ...
            for l in L:
                ...
                CL[i,j,l] = np.real((integrate.cumtrapz(I, k, initial=0))[-1])
...
def CL_bins(T_list1, T_list2, k, L, As=conf.As, ns=conf.ns):
    ...
    for i in np.arange(len(T_list1)):
        for l in L:
            ...
            CL[i,l] = np.real(integrate.trapz(I, k))
        ...
        if l >= l_min:
            ...
            for i in np.arange(len(Transfers)):
                ...
                for j in np.arange(i,len(Transfers)):
                    ...
                    CL[i,j] = np.real((integrate.cumtrapz(I, k, initial=0))[-1])
...
def get_CLvv_cov_localdoppler():
    ...
    for i in np.arange(conf.N_bins):
        ...
        T_list.append(Transfer_ksz_bin_localdoppler(conf.N_bins, conf.n_samples, i, k, Lv, conf.Omega_b, conf.Omega_c, conf.w, conf.wa, conf.Omega_K, conf.h))
...
def get_CLvv_cov_nolocaldopp():
    ...
    for i in np.arange(conf.N_bins):
        ...
        T_list.append(Transfer_ksz_bin_nolocaldoppler(conf.N_bins, conf.n_samples, i, k, Lv, conf.Omega_b, conf.Omega_c, conf.w, conf.wa, conf.Omega_K, conf.h))
...
def get_CLvv_cov_arrays():
    ...
    for i in np.arange(conf.N_bins):
        ...
        T_list.append(Transfer_ksz_bin_arrays(conf.N_bins, conf.n_samples, i, k, Lv, conf.Omega_b, conf.Omega_c, conf.w, conf.wa, conf.Omega_K, conf.h))
...
def get_CLvv_cov_arrays2():
    ...
    for i in np.arange(conf.N_bins):
        ...
        T_list.append(Transfer_ksz_bin_arrays2(conf.N_bins, conf.n_samples, i, k, Lv, conf.Omega_b, conf.Omega_c, conf.w, conf.wa, conf.Omega_K, conf.h))
